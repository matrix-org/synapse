...
class CapabilitiesTestCase(unittest.HomeserverTestCase):
    ...
    def test_get_does_not_include_msc2716_fields_when_disabled(self) -> None:
        access_token = self.get_success(
            self.auth_handler.create_access_token_for_user_id(
                self.user, device_id=None, valid_until_ms=None
            )
        ...
        self.assertEqual(channel.code, 200)
        self.assertNotIn(
            "org.matrix.msc2716.room_capabilities", capabilities["m.room_versions"]
        )

    @override_config({"experimental_features": {"msc2716_enabled": True}})
    def test_get_does_include_msc2716_fields_when_enabled(self) -> None:
        access_token = self.get_success(
            self.auth_handler.create_access_token_for_user_id(
                self.user, device_id=None, valid_until_ms=None
            )
