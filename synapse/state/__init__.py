...
class StateHandler:
    ...
    async def compute_event_context(
        ...
        if not state_group_before_event:
            state_group_before_event = (
                await self.state_storage_controller.store_state_group(
                    event.event_id,
                    event.room_id,
        ...
        state_group_after_event = await self.state_storage_controller.store_state_group(
            event.event_id,
            event.room_id,
    ...
    async def resolve_state_groups_for_events(
        ...
        state_groups = await self.state_storage_controller.get_state_group_for_events(
            event_ids
        )
        ...
        if len(state_group_ids_set) == 1:
            ...
            state = await self.state_storage_controller.get_state_for_groups(
                state_group_ids_set
            )
            (
                prev_group,
                delta_ids,
            ) = await self.state_storage_controller.get_state_group_delta(
                state_group_id
            )
        ...
        state_to_resolve = await self.state_storage_controller.get_state_for_groups(
            state_group_ids_set
        )
