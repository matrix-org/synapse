...
class EventAuthHandler:
    ...
    async def check_user_in_room(
        ...
        if current_state:
            ...
            member = await self._state_handler.get_current_state(
        ...
        if member:
            ...
            if allow_departed_users and membership == Membership.LEAVE:
                forgot = await self._store.did_forget(user_id, room_id)
                if not forgot:
                    return member
    ...
    async def check_user_in_room_or_world_readable(
        ...
        try:
            ...
            visibility = await self._state_handler.get_current_state(
    ...
    async def check_can_change_room_list(self, room_id: str, user: UserID) -> bool:
        ...
        is_admin = await self._store.is_server_admin(user)
        if is_admin:
            return True
        ...
        power_level_event = await self._state_handler.get_current_state(
