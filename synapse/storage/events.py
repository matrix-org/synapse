...
class EventsStore(SQLBaseStore):
    ...
    def persist_events(self, events_and_contexts, backfilled=False):
        ...
        deferreds = []
        for room_id, evs_ctxs in list(partitioned.items()):
            ...
            for chunk in chunks:
                ...
                if not backfilled:
                    with Measure(self._clock, "_calculate_state_and_extrem"):
                        ...
                        for room_id, ev_ctx_rm in list(events_by_room.items()):
                ...
                for room_id, (_, _, new_state) in list(current_state_for_room.items()):
                    self.get_current_state_ids.prefill(
                        (room_id, ), new_state
                    )

                for room_id, latest_event_ids in list(new_forward_extremeties.items()):
    ...
    def _calculate_state_delta(self, room_id, events_context, new_latest_event_ids):
        ...
        if missing_event_ids:
            ...
            groups = set(event_to_groups.values()) - state_groups

            if groups:
                group_to_state = yield self._get_state_for_groups(groups)
                state_sets.extend(iter(list(group_to_state.values())))
        ...
        existing_events = set(existing_state.values())
        new_events = set(ev_id for ev_id in list(current_state.values()))
        ...
        to_delete = {
            key: ev_id for key, ev_id in list(existing_state.items())
            if ev_id in changed_events
        }
        events_to_insert = (new_events - existing_events)
        to_insert = {
            key: ev_id for key, ev_id in list(current_state.items())
    ...
    def _update_forward_extremities_txn(self, txn, new_forward_extremities,
                                        max_stream_order):
        for room_id, new_extrem in list(new_forward_extremities.items()):
    ...
    def _update_room_depths_txn(self, txn, events_and_contexts, backfilled):
        ...
        for room_id, depth in list(depth_updates.items()):
        ...
        for sg in remaining_state_groups:
            ...
            self._simple_insert_many_txn(
                ...
                values=[
                    ...
                    for key, state_id in list(curr_state.items())
                ],
            )
